...
(function() {
  ...
  class NEXXCore {
    ...
    async init() {
      ...
      // Предзагружаем чанки базы — D1 API (с фолбеком на статические файлы)
      await this.preload([
        '/api/d1/chunks/config.json',
        '/api/d1/chunks/devices.json',
        '/api/d1/chunks/prices.json',
        '/api/d1/chunks/services.json'
      ]);
  ...
  class ErrorBoundary {
    constructor(core) {
      ...
      this.GRACE_MS = 3500; // не показывать тост при ошибках в первые 3.5 сек (обновление/загрузка)
    ...
    _isBenignError(message) {
      ...
      return (
        ...
        s.includes('non-error promise') ||
        s.includes('aborted') ||
        s.includes('failed to fetch') && (Date.now() - this.loadTime < 5000)
      );
    }
    ...
    handleError(error) {
      ...
      // Не показывать тост: в период после загрузки или известные «тихие» ошибки
      if (Date.now() - this.loadTime < this.GRACE_MS) return;
      if (this._isBenignError(message)) return;
