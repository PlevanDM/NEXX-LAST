...
(function() {
  ...
  const QuickActions = ({ currentPage }) => {
    ...
    const actions = [
      { id: 'home', label: 'На головну', icon: 'fa-house', gradient: 'from-gray-900 to-gray-800', path: '/' },
      { id: 'calculator', label: 'Калькулятор', icon: 'fa-calculator', gradient: 'from-gray-800 to-gray-700', path: '/calculator' },
      { id: 'database', label: 'База даних', icon: 'fa-database', gradient: 'from-gray-700 to-gray-600', path: '/nexx.html' },
      { id: 'phone', label: 'Дзвінок', icon: 'fa-phone', gradient: 'from-gray-800 to-gray-900', path: 'tel:+380000000000' },
      { id: 'telegram', label: 'Telegram', icon: 'fa-telegram', gradient: 'from-gray-700 to-gray-800', path: 'https://t.me/nexx_support' }
    ];
      ...
      h('div', { className: 'flex flex-col items-end gap-3' },
        ...
        isExpanded && visibleActions.map((action, idx) => 
          h('a', {
            ...
            h('span', { className: 'font-medium text-sm' }, action.label),
        ...
        h('button', {
          ...
          title: isExpanded ? 'Закрити' : 'Швидкі дії'
        },
