...
const App = () => {
  ...
  // Load all data from chunks (config, devices, prices, knowledge)
  useEffect(() => {
    const opts = { credentials: 'include' };
    Promise.all([
      fetch('/data/chunks/config.json', opts).then(r => r.ok ? r.json() : {}),
      fetch('/data/chunks/devices.json', opts).then(r => r.ok ? r.json() : []),
      fetch('/data/chunks/prices.json', opts).then(r => r.ok ? r.json() : {}),
      fetch('/data/chunks/knowledge.json', opts).then(r => r.ok ? r.json() : {})
    ])
      .then(([config, devices, prices, knowledge]) => {
        // Устройства
        setDevices(Array.isArray(devices) ? devices : []);
        
        // Цены
        if (prices && Object.keys(prices).length) {
          setUkrainePrices(prices);
        }
        
        // База знаний (из chunk knowledge)
        const k = knowledge || {};
        
        // Логические платы
        setLogicBoardsSpecs(k.logicBoards?.specs || {});
        
        // IC совместимость
        setIcData(k.icCompatibility || {});
        
        // Коды ошибок
        setErrorData(k.errorCodes || {});
        
        // База знаний по ремонту
        setKnowledgeData(k.repairKnowledge || {});
        
        // Региональные коды
        setRegionData(k.regionalCodes?.basic || {});
        
        // Комбинации клавиш
        setKeyCombinations(k.keyCombinations || {});
        
        // Индекс статей
        setArticlesIndex(k.articleSearchIndex || {});
        
        // Измерения
        setMeasurementsData(k.measurements || {});
        
        // Расширенные данные для глобального доступа
        window.NEXXExtendedData = {
          cameraCompatibility: k.cameraCompatibility,
          regionalCodesExtended: k.regionalCodes?.extended
        };
        
        setLoading(false);
        console.log('✅ NEXX Database v3.0 - загружено из чанков');
        console.log('📊 Данные:', { 
          devices: (devices || []).length,
          knowledge: Object.keys(k).length
        });
      })
      .catch(err => {
