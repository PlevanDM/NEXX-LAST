...
const App = () => {
  ...
  // Load all data from D1 API (fallback to static chunks)
  useEffect(() => {
    const opts = { credentials: 'include' };
    const fc = (n) => fetch('/api/d1/chunks/' + n + '.json', opts).then(r => r.ok ? r.json() : null).catch(() => null).then(d => d !== null ? d : fetch('/data/chunks/' + n + '.json', opts).then(r => r.ok ? r.json() : (n === 'devices' ? [] : {})));
    Promise.all([
      fc('config'),
      fc('devices'),
      fc('prices'),
