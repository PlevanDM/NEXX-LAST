 ...
 * Автоматически заменяет все Font Awesome иконки на inline SVG во всех файлах
 */
...
// Полный маппинг Font Awesome -> Lucide SVG (inline)
const faToLucideSVG = {
  // Navigation
...
// Функция для замены Font Awesome иконок
function replaceFontAwesome(content, filePath) {
  ...
  Object.entries(faToLucideSVG).forEach(([faClass, svg]) => {
    ...
    // Паттерн 1: <i className="fas fa-NAME" ...> или <i class="fas fa-NAME" ...> (JSX)
    ...
    if (pattern1.test(newContent)) {
      newContent = newContent.replace(pattern1, (match) => {
        // Извлекаем классы для размера и цвета
        ...
        // Обновляем SVG с правильными классами
    ...
    // Паттерн 2: h('i', { className: 'fas fa-NAME' }) - простой случай
    ...
    // Паттерн 3: icon: 'fa-NAME' в объектах - заменяем напрямую в массивах
    const pattern3 = new RegExp(`icon:\\s*['"]fa-${faName}['"]`, 'gi');
    if (pattern3.test(newContent)) {
      // Оставляем как есть, но отмечаем для ручной обработки
      modified = true;
    }
  });
...
// Основная функция
function main() {
  log('\n🔧 Автоматическая замена Font Awesome на Lucide SVG...', 'cyan');
  log('='.repeat(60), 'cyan');
  ...
  log(`\n📁 Найдено файлов для проверки: ${files.length}`, 'cyan');
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Проверяем, есть ли Font Awesome
      if (!/fa[sb]?\s+fa-/.test(content)) {
        continue; // Пропускаем файлы без FA
      }
      
      // Отладка: показываем первые совпадения
      const faMatches = content.match(/fa[sb]?\s+fa-[a-z-]+/gi);
      if (faMatches && faMatches.length > 0) {
        log(`   🔍 ${file}: найдено ${faMatches.length} иконок (примеры: ${faMatches.slice(0, 3).join(', ')})`, 'yellow');
      }
      ...
      if (modified && replacements > 0) {
        ...
        log(`   ✅ ${file} (${replacements} замен)`, 'green');
      }
    } catch (e) {
  ...
  // Итоги
  log('\n' + '='.repeat(60), 'cyan');
  log(`✅ Исправлено файлов: ${fixed.length}`, 'green');
  log(`📊 Всего замен: ${totalReplacements}`, 'green');
  
  if (fixed.length > 0) {
    log('\n📝 Исправленные файлы:', 'cyan');
    fixed.forEach(f => log(`   - ${f.file} (${f.replacements} замен)`, 'gray'));
  }
  
  if (errors.length > 0) {
    log(`\n❌ Ошибок: ${errors.length}`, 'red');
    errors.forEach(e => log(`   - ${e.file}: ${e.error}`, 'red'));
  }
  
  log('\n💡 Примечание: Некоторые паттерны (template literals) могут требовать ручной доработки', 'yellow');
  log('');
}
