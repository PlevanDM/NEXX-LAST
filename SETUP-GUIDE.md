...
- `DATABASE-UPDATE-GUIDE.md` - Руководство по обновлению базы данных
...
Файл содержит список доступных услуг с их иконками, популярностью и категориями совместимости.

### Структура `prices.json`
Файл содержит ценовые диапазоны (`min`, `max`, `avg`) для каждой категории устройств (phone, tablet, laptop).

Для обновления цен редактируйте `public/data/master-db.json` и запускайте `npm run split-db`.
